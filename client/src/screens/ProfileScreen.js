...
import {FaUser,FaHeart, FaBookOpen} from 'react-icons/fa'
...
try {
  ...
  console.log(err)
    ...
    <p className="text-sm text-center text-gray-400">Soyez le bienvenue dans notre communaute,sachez que cest un plaisir pour nous de vous accueilir</p>
            ...
            {option ===3 ? ( <div className="flex flex-col flex-3 flex-wrap">
              {userArticles.map((article) => (
                <UserArticle title={article.Title} date={article.publishedDate} description={article.Description} articleId={article._id} />
              ))}
              </div>
            ...
            {option ===2 ? ( <div className="flex flex-col flex-3 flex-wrap">
              <h1>Aritcles like</h1>
              </div>
